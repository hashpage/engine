...
/*!
 * jTemplates 0.7.5 (http://jtemplates.tpython.com)
 * Copyright (c) 2007-2008 Tomasz Gloc (http://www.tpython.com)
 * 
 * Dual licensed under the MIT (MIT-LICENSE.txt)
 * and GPL (GPL-LICENSE.txt) licenses.
 *
 * Id: $Id: jquery-jtemplates_uncompressed.js 169 2008-08-31 17:36:12Z tom $
 */
 ...
 * @name jTemplates
 * @author Tomasz Gloc
 * @date $Date: 2008-08-31 19:36:12 +0200 (N, 31 sie 2008) $
 */
...
if(window.jQuery && !window.jQuery.createTemplate) {(function() {
     ...
     * @type string
     */
    Template.prototype.version = '0.7.5';
        ...
        for(var i=0, l=(op)?(op.length):(0); i<l; ++i) {
            ...
            switch(op_) {
                ...
                case '/foreach':
                    ...
                    break;
                case 'foreach':
                    e = new opFOREACH(op[i], node, this);
                    node.push(e);
                    node = e;
                    break;
                case 'for':
                    e = opFORFactory(op[i], node, this);
                    node.push(e);
                    node = e;
                    break;
                case 'include':
        ...
        try {
            ...
            if((e>s && step>0) || (e<s && step<0)) {
                ...
                for(; ((step>0) ? (s<e) : (s>e)); s+=step, ++iteration) {
                    ...
                    $T[this._name + '$key'] = (ckey !== undefined && ckey.constructor == String) ? (this._template.f_escapeString(ckey)) : (ckey);
    ...
    var Include = function(oper, includes) {
        ...
        var name = RegExp.$1;
        var root = RegExp.$2;
        this._template = includes[name];
        if(this._template == undefined) {
            if (includes['~']) { // try to lookup template dynamically
                this._template = includes['~'](name);
            }
        }
        if(this._template == undefined) {
            if(Template.DEBUG_MODE)
                throw new Error('jTemplates: Cannot find include: ' + name);
        }
        this._root = root;
    };
